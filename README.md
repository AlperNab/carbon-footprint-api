...
**Domain:** `ESG / Sustainability` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Carbon Footprint Api** is a standalone, browser-based AI workflow app for **ESG / Sustainability**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `activity_data`<br>Activity data | text | Yes | — | Affects input: Activity data. |
...
| `emission_factors`<br>emission factors | text | No | — | Affects customization: emission factors. |
| `units`<br>units | text | No | — | Affects customization: units. |
| `boundary_rules`<br>boundary rules | textarea | No | — | Affects customization: boundary rules. |
| `uncertainty_tolerance`<br>uncertainty tolerance | slider | No | 50 | Affects customization: uncertainty tolerance. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9105
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9105`. |
...
- Export download
- Job history
- Country/year emission-factor database optional
...
```bash
curl http://127.0.0.1:9105/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9105/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9105 uvicorn app.main:app --host 0.0.0.0 --port 9105
```
...
- `plugins/carbon-footprint-api.json` for plugin metadata.
